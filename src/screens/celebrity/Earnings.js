...
export default function Earnings() {
  ...
  const StatCard = ({ icon, label, value, prefix = 'ZWG', isCurrency = true }) => (
    <View style={styles.card}>
      <View style={styles.cardRow}>
        <Ionicons name={icon} size={26} color={Colors.primary} style={styles.cardIcon} />
  ...
  return (
    <ScrollView style={styles.scroll} contentContainerStyle={styles.container}>
      ...
      <StatCard icon="bag-add-outline" label="Pending Requests" value={earnings.pending} isCurrency={false} />
      <StatCard icon="checkmark-done-circle-outline" label="Completed Shoutouts" value={earnings.completed} isCurrency={false} />
...
const styles = StyleSheet.create({
  scroll: { flex: 1, backgroundColor: '#fff9f5' },
  container: { padding: 24, paddingBottom: 40 },
  backgroundImage: { ...StyleSheet.absoluteFillObject, opacity: 0.05 },
  heading: {
    fontSize: 26,
    fontWeight: '800',
    color: Colors.primary,
  ...
  amount: { fontSize: 20, fontWeight: '700', color: '#222' },
});
